...
fn why_json_full(c: &Sandbox, id: &str) -> Value {
...
// ---------------------------------------------------------------------------
// `d551`: a pillar or rule that is no longer open carries a mark in the
// prose and its state in the JSON; an open one carries no mark, and a late
// declaration keeps both its mark and its date.
// ---------------------------------------------------------------------------

#[test]
fn against_marks_a_target_that_closed_or_was_abandoned_and_leaves_an_open_one_bare() {
    let c = with_open_rule("against-state-marks");
    c.ok(&[
        "add",
        "Close the loop before the write",
        "--parent",
        "1",
        "--type",
        "rule",
        "--why",
        "a second rule under the pillar",
    ]); // r3
    c.ok(&[
        "add",
        "Never wait on a lock across the network",
        "--parent",
        "1",
        "--type",
        "rule",
        "--why",
        "a third rule under the pillar",
    ]); // r4
    c.ok(&[
        "decide",
        "Keep it local",
        ...
        "the pillar settles it",
        "--against",
        "p1: the pillar still governs",
        "--against",
        "r3: the write stays under 5 ms",
    ]); // d5
    c.ok(&[
        "declare",
        "5",
        "--against",
        "r4: nothing on the write path calls the network",
    ]);
    c.ok(&["done", "3", "measured and kept"]);
    c.ok(&["abandon", "4", "no longer applies"]);

    let why = c.ok(&["why", "5"]);
    assert!(
        why.contains("judged against p1: the pillar still governs"),
        "an open target must carry no mark:\n{why}"
    );
    assert!(!why.contains("judged against p1 ["), "{why}");
    assert!(
        why.contains("judged against r3 [closed]: the write stays under 5 ms"),
        "a closed target should carry [closed]:\n{why}"
    );
    assert!(
        why.contains("judged against r4 [abandoned]:") && why.contains("(declared "),
        "a late declaration against an abandoned target should carry both its mark and its date:\n{why}"
    );

    let v = why_json(&c, "5");
    let against = v["node"]["against"].as_array().expect("against is a list");
    assert_eq!(against.len(), 3, "{v}");
    assert_eq!(against[0]["node"], "p1", "{v}");
    assert_eq!(against[0]["state"], "active", "{v}");
    assert_eq!(against[1]["node"], "r3", "{v}");
    assert_eq!(against[1]["state"], "done", "{v}");
    assert_eq!(against[2]["node"], "r4", "{v}");
    assert_eq!(against[2]["state"], "abandoned", "{v}");
    assert!(!against[2]["declared"].is_null(), "{v}");
}

// ---------------------------------------------------------------------------
// `d469`: `against` reaches a decision that is a step of the path, and only
// under `--full`, which is `d330`'s rule for the prose.
// ---------------------------------------------------------------------------

#[test]
fn against_reaches_an_ancestor_step_of_the_path_only_under_full() {
    let c = with_open_rule("against-path-full");
    c.ok(&[
        "decide",
        "Keep it local",
        "--reason",
    ...
    let full = c.ok(&["why", "4", "--full"]);
    assert!(
        full.contains("judged against r2: nothing on the write path calls the network"),
        "{full}"
    );
    let plain = c.ok(&["why", "4"]);
    assert!(
        !plain.contains("judged against"),
        "an ancestor's declaration leaked without --full:\n{plain}"
    );

    let full_json = why_json_full(&c, "4");
    let step = &full_json["path"][0];
    assert_eq!(step["alias"], "d3", "{full_json}");
    let against = step["against"]
        .as_array()
        .expect("against is a list under --full");
    assert_eq!(against.len(), 1, "{full_json}");
    assert_eq!(against[0]["node"], "r2", "{full_json}");

    let plain_json = why_json(&c, "4");
    assert!(
        plain_json["path"][0].get("against").is_none(),
        "against leaked into the plain path step:\n{plain_json}"
    );
}
