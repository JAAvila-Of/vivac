...
pub fn start(ctx: &mut crate::ops::Ctx, a: &Args, project: &str) -> R {
    if !a.has("hook") {
        return crate::brief::brief(&ctx.tree, &ctx.store.root, ctx.anchor.as_ref(), a, project);
    }
    ...
    let text = crate::brief::to_text(&ctx.tree, &ctx.store.root, ctx.anchor.as_ref(), a, project)?;
    print!("{text}");
